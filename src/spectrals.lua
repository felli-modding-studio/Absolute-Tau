    ...
    func = function(card, layer)
        if (layer == 'card' or layer == 'both') and card.sprite_facing == 'front' then
            if card.config.center.set == "TauSpec" then
                card.children.center:draw_shader('tau_inverse_booster', nil, card.ARGS.send_to_shader)
...
local tauspecs = {
            ...
            for i = 1, cards do
                G.E_MANAGER:add_event(Event({
                    func = function()
                        SMODS.add_card({
                            set = "Enhanced",
                            edition = poll_edition("tau_grim", nil, true, true),
                            rank = "Ace",
                            area = G.hand,
                        })
                        return true
    ...
    },
    {
        ...
        loc_vars = function(self, info_queue, card)
            ...
            info_queue[#info_queue + 1] = G.P_SEALS.tau_Gilded
            return {
                vars = {
    ...
    },
    {
        ...
        use = function(self, card, area, copier)
            for _, c in pairs(G.hand.highlighted) do
                c:set_edition(poll_edition("tau_aura", 5, nil, true))
                G.E_MANAGER:add_event(Event({
                    func = function()
                        c.area:remove_from_highlighted(c)
                        return true
                ...
                func = function()
                    for _, c in pairs(G.playing_cards) do
                        G.E_MANAGER:add_event(Event({
                            ...
                            func = function()
                                SMODS.change_base(c, pseudorandom_element(SMODS.Suits, "tau_sigil").key)
                                c:juice_up()
                    ...
                    for _, c in pairs(G.playing_cards) do
                        if c:is_suit(mps) then
                            G.E_MANAGER:add_event(Event({
                                ...
                                func = function()
                                    SMODS.change_base(c,
                                        pseudorandom_element(SMODS.Suits, "tau_sigil_rerandomize").key)
                                    c:juice_up()
                ...
                func = function()
                    for _, c in pairs(G.playing_cards) do
                        G.E_MANAGER:add_event(Event({
                            ...
                            func = function()
                                SMODS.change_base(c, nil, pseudorandom_element(SMODS.Ranks, "tau_ouija").key)
                                c:juice_up()
                            ...
                            G.E_MANAGER:add_event(Event({
                                ...
                                func = function()
                                    SMODS.change_base(c, nil,
                                        pseudorandom_element(SMODS.Ranks, "tau_ouija_rerandomize").key)
                                    c:juice_up()
    ...
    },
    {
        ...
        loc_vars = function(self, info_queue, card)
            ...
            info_queue[#info_queue + 1] = G.P_SEALS.tau_Entropic
            return {
                vars = {
                    card.ability.extra.max, card.ability.extra.min
                }
    ...
    },
    {
        ...
        use = function(self, card, area, copier)
            for _,joker in pairs(G.jokers.cards) do
                joker:set_edition(poll_edition("tau_hex_roll",nil,nil,true))
            end
            SMODS.destroy_cards({ pseudorandom_element(G.jokers.cards, "tau_hex_destroy") })
        end,
    },
    {
        original = "c_trance",
        ...
        loc_vars = function(self, info_queue, card)
            ...
            info_queue[#info_queue + 1] = G.P_SEALS.tau_Galactic
            return {
                vars = {
                    card.ability.extra.max, card.ability.extra.min
                }
    ...
    },
    {
        ...
        loc_vars = function(self, info_queue, card)
            ...
            info_queue[#info_queue + 1] = G.P_SEALS.tau_Vibrant
            return {
                vars = {
                    card.ability.extra.max, card.ability.extra.min
                }
