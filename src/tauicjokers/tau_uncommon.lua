        ...
        if context.setting_blind then
            ...
            if my_pos and G.jokers.cards[my_pos + 1] and not card.getting_sliced and not G.jokers.cards[my_pos + 1].ability.eternal and not G.jokers.cards[my_pos + 1].getting_sliced then
                ...
                card_eval_status_text(card, 'extra', nil, nil, nil,
                    { message = localize { type = 'variable', key = 'a_mult', vars = { card.ability.mult + sliced_card.sell_cost } }, colour =
...
AbsoluteTau.Tauic {
    ...
    calculate = function(self, card, context)
        ...
        if (context.using_consumeable) and SMODS.pseudorandom_probability(card, 'tau_oa6', card.ability.extra.base, card.ability.extra.chance, 'oa6.28') then
            G.jokers:change_size(card.ability.extra.slots)
...
AbsoluteTau.Tauic {
    ...
    calculate = function(self, card, context)
        if (context.using_consumeable and SMODS.pseudorandom_probability(card, "tau_cartomancer", card.ability.extra.min, card.ability.extra.prob)) then
